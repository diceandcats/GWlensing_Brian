...
class ClusterLensing(ClusterLensingUtils):
    ...
    def calculate_time_delay_uncertainty(self, img: np.ndarray, index: int) -> np.ndarray:
        ...
        pix = float(self.data.pixscale[index])               # arcsec / pixel
        # img is [x_img, y_img] in arcsec; convert to pixel indices
        x_pix = np.asarray(img[0], dtype=np.float64) / pix
        y_pix = np.asarray(img[1], dtype=np.float64) / pix

        n_x, n_y = sigma_dt.shape
        points = (np.arange(n_y), np.arange(n_x))                # (y, x) index axes
        # interpn expects (y, x) order for 2D arrays
        xi = np.vstack((y_pix, x_pix)).T

        sigma_dt_values = interpn(points, sigma_dt, xi, method='linear',
                                bounds_error=True)
    ...
    def run_de_optimization(self,
        ...
        default_settings = {
            'strategy': 'rand1bin', 'maxiter': 180, 'popsize': 50, 'tol': 1e-7,
        ...
        try:
            ...
            n_available = len(os.sched_getaffinity(0))//2
        except Exception:
