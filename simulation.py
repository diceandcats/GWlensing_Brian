...
sample_failed = len(mcmc_results) == 0
if sample_failed:
    print("MCMC sampling failed for all clusters; no results to process.")
    updates = {
    ...
    if should_plot:
        ...
        if not os.path.exists(data_file):
            ...
            truth_values = [real_params['x_src'], real_params['y_src']]
            if 'z_s' in param_labels and 'z_s' in real_params:
                truth_values.append(real_params['z_s'])
            if 'H0' in param_labels and 'H0' in real_params:
                truth_values.append(real_params['H0'])
